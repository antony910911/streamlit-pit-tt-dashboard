...
st.set_page_config(
    ...
    page_icon="images/logo.png",  # 圖檔路徑（建議 PNG, 32x32 或 64x64）
    layout="wide",
...
with tabs[0]:
    ...
    **注意事項：**  
    ...
    ---
    以下是針對“PIT/TT 多日變化趨勢比對”基本操作指南：
    
    1. 選擇要比對的日期（可多選，最多顯示近 14 日內資料）

    2. 選擇比對欄位（支援 PIT-* 或 TT-* 欄位）

    3. 調整取樣間隔（預設 30 秒，可選 5 秒至 15 分鐘）


    4. 上傳氣象資料 CSV（選用）：

        支援中央氣象局「柳營氣象站（C0X320）」的格式

        自動辨識 MH 格式或標準格式

        勾選「顯示柳營氣溫曲線」並上傳檔案後，即可同時呈現氣溫變化

        若成功繪出氣溫，圖表上方會顯示副標題說明
